...
public class GamePowerUnlimited {
    ...
    /** ArrayList with all possible combo position for the IA */
     ...
     * @param xLength int - grid axe X length (without border)
     * @param yLength int - grid axe Y length (without border)
     ...
     * @param dir Direction - direction for movement
    ...
    private boolean checkComboLine(int iPosX, int iPosY, Direction dir, PieceType pieceType, boolean isOpposite) {
        ...
        for (int j = 0; j < this.comboLength; j++) {
            ...
            if (this.isPieceAt(iPosX, iPosY, pieceType)) {
                ...
                if (!isOpposite && countCombo > 1) {
                    return this.checkComboLine(
    ...
    /**
     * Insert a new piece. The method create a recursion for make the piece fall.
     *
     ...
     *
     * @param posX   the position on the X (with barrier)
     * @param posY   the position on the Y (with barrier)
     * @param pieces all pieces for the equals
     * @return true if the pieces equals with cell data
     */
    public boolean isPieceAt(int posX, int posY, PieceType... pieces) {
        for (PieceType piece : pieces)
            return piece.equals(this.grid[posX][posY]);
        return false;
    }
    ...
    public String toString() {
        ...
        for (int x = 0; x < this.xLengthWithBorder; x++) {
            ...
            if (x == this.lastPositionPieceAddedX) {
                ...
                for (int y = 0; y < this.yLengthWithBorder; y++)
                    sb.append(y == this.lastPositionPieceAddedY ? AnsiColor.RED.apply(String.valueOf(this.grid[x][y].ordinal())) : this.grid[x][y].ordinal()).append(",");
            } else {
                // normal
                for (int y = 0; y < this.yLengthWithBorder; y++)
